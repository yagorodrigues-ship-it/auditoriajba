...
# --- TELA LOGADA DO SISTEMA ---
            ...
            if codigo_input:
                ...
                if not item_autorizado:
                    ...
                    if not itens_filtrados.empty:
                        ...
                        # --- SAFE ATTRIBUTE VERIFICATION CHECK ---
            ...
            with st.expander("➕ Nova Pasta de Auditoria do Supervisor"):
                with st.form("form_novo_sup_interno_v4", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Criar Pasta Supervisor", type="primary") and nome_sup_inv:
                        ...
                        cursor = conn.cursor()
                        cursor.execute("INSERT INTO inventarios_supervisor (id, nome, data, status) VALUES (?, ?, ?, 'Aberto')", (novo_id_sup, nome_sup_inv, today_sup := hoje_sup))
                        conn.commit()
                        st.rerun()
    ...
    with aba_acuracidade:
        ...
        # --- FIXED COLUMN AND NAME VARIABLE ORDER DECLARATION (ABAS DE DATA ANTECIPADAS) ---
