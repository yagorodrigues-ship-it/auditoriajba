...
def encontrar_coluna(df, opcoes, default_idx):
    if df is not None:
...
# --- TELA LOGADA DO SISTEMA ---
    ...
    col_cod = encontrar_coluna(st.session_state.base_sistema, ['códproduto', 'codproduto', 'codigo', 'cod'], 0)
    col_desc = encontrar_coluna(st.session_state.base_sistema, ['descproduto', 'descricao', 'desc'], 1)
    col_local = encontrar_coluna(st.session_state.base_sistema, ['descestoquefisico', 'localizacao', 'local', 'estoquefisico'], 2)
    col_unidade = encontrar_coluna(st.session_state.base_sistema, ['unidmedida', 'unidade', 'un'], 3)
    col_qtd = encontrar_coluna(st.session_state.base_sistema, ['qtdestoque', 'quantidade', 'saldo', 'qtd'], -1)
    col_id_estoque = encontrar_coluna(st.session_state.base_sistema, ['idestoquefísico', 'idestoqfísico', 'idestoque', 'codestoque'], 0)
    ...
    # --- DEFINIÇÃO DA NOVA ORDEM EXATA DAS ABAS SOLICITADAS ---
    ...
    # ------------------------------------------------------------------------------------------------------
    # 🔍 ABA 1: CONTAR ITEM (COM RECONHECIMENTO SE O ATIVO EXISTE NA BASE)
    # ------------------------------------------------------------------------------------------------------
    with abas_render[0]:
        if id_inventario_atual is None or st.session_state.base_sistema is None:
            ...
            if codigo_input:
                ...
                if not item.empty:
                    ...
                    # --- VALIDAÇÃO SE O ATIVO JÁ CONSTA NO EXCEL ---
                    col_ativo_check = encontrar_coluna(st.session_state.base_sistema, ['ativo', 'patrimonio', 'n ativo'], -1)
                    tem_ativo_na_base = False
                    if col_ativo_check in item.columns:
                        val_at = str(item.iloc[0][col_ativo_check]).strip()
                        if val_at and val_at.lower() != "nan" and val_at != "":
                    ...
                    with st.form("confirmar_form", clear_on_submit=True):
                        ...
                        # Ativo obrigatório apenas se constar na planilha carregada
                        if tem_ativo_na_base:
                            ativo_input = st.text_input("🔢 Número do Ativo (OBRIGATÓRIO PARA ESTE ITEM)")
                        else:
                        ...
                        if st.form_submit_button("✓ Confirmar Contagem", type="primary", use_container_width=True):
                            ...
                            elif tem_ativo_na_base and not ativo_l:
                                st.error("❌ Erro: O preenchimento do Ativo é obrigatório para este material, pois ele consta no saldo da planilha!")
                            else:
    ...
    # --- ABA 5: DESEMPENHO E PRAZOS (COMPLETAMENTE DINÂMICO) ---
    with abas_render[4]:
    ...
    # --- RECURSOS EXCLUSIVOS DO CARGO SUPERVISOR ---
    if eh_supervisor:
        # 🔬 ABA 6: PAINEL SUPERVISOR (ATUALIZADA E 100% OPERACIONAL CONFORME IMAGE_E1CFDB.PNG)
        with abas_render[5]:
            ...
            if df_inventarios_sup.empty:
                ...
                inv_sup_selected = st.selectbox("Selecione a sua Auditoria Amostral", lista_inv_sup, key="sel_box_sup_local_v4")
            ...
            # --- IMPLEMENTAÇÃO SOLICITADA: BIPAGEM, CONTROLE DE ATIVO E ALIMENTAÇÃO DA ACURACIDADE ---
            if id_inv_sup_atual and 'inv_sup_selected_obj' in locals() and inv_sup_selected_obj['status'] == 'Aberto' and st.session_state.base_supervisor is not None:
                ...
                col_cod_sup = encontrar_coluna(st.session_state.base_supervisor, ['códproduto', 'codproduto', 'codigo', 'cod'], 0)
                col_desc_sup = encontrar_coluna(st.session_state.base_supervisor, ['descproduto', 'descricao', 'desc'], 1)
                col_local_sup = encontrar_coluna(st.session_state.base_supervisor, ['descestoquefisico', 'localizacao', 'local', 'estoquefisico'], 2)
                col_qtd_sup = encontrar_coluna(st.session_state.base_supervisor, ['qtdestoque', 'quantidade', 'saldo', 'qtd'], -1)
                col_id_est_sup = encontrar_coluna(st.session_state.base_supervisor, ['idestoquefísico', 'idestoqfísico', 'idestoque', 'codestoque'], 0)
                col_atv_sup = encontrar_coluna(st.session_state.base_supervisor, ['ativo', 'patrimonio', 'n ativo'], -1)

                bip_sup_item = st.text_input("💻 Bipar Produto para Auditoria Amostral", key="bip_sup_action")
                if bip_sup_item:
                    sup_bl = str(bip_sup_item).strip().upper()
                    match_sup = st.session_state.base_supervisor[st.session_state.base_supervisor[col_cod_sup].astype(str).str.upper().str.strip() == sup_bl]
                    ...
                    if not match_sup.empty:
                        ...
                        # Ativo obrigatório apenas se constar na planilha base carregada
                        tem_ativo_sup = False
                        if col_atv_sup in match_sup.columns:
                            v_at_s = str(row_sup[col_atv_sup]).strip()
                            if v_at_s and v_at_s.lower() != "nan" and v_at_s != "":
                                tem_ativo_sup = True
                        ...
                        with st.form("form_auditoria_action", clear_on_submit=True):
                            ...
                            if tem_ativo_sup:
                                f_ativo_sup = st.text_input("🔢 Número do Ativo (OBRIGATÓRIO)")
                            else:
                            ...
                            if st.form_submit_button("💾 Salvar na Acuracidade", type="primary"):
                                if tem_ativo_sup and not f_ativo_sup.strip():
                                    ...
                                    conn.commit()
                                    st.success("✅ Dados de acuracidade processados e salvos!")
                                    st.rerun()
                    else:
                        st.error("❌ Produto não localizado na planilha base de amostragem.")
        ...
        with abas_render[6]:
            ...
            if df_ac.empty:
                st.info("Nenhuma amostragem de acuracidade coletada no banco de dados.")
            else:
                total_aud = len(df_ac)
                certos_etiq = len(df_ac[df_ac['etiqueta_correta'] == "Sim"])
                certos_local = len(df_ac[df_ac['localizacao_correta'] == "Sim"])
                st.metric("Total de Amostras Bipadas pelo Supervisor", total_aud)
                c_ac1, c_ac2 = st.columns(2)
                c_ac1.metric("Acuracidade de Etiquetas (Identificação OK)", f"{(certos_etiq / total_aud)*100:.1f}%")
                c_ac2.metric("Acuracidade de Localização (Alocação OK)", f"{(certos_local / total_aud)*100:.1f}%")
                st.dataframe(df_ac.drop(columns=['unidade'], errors='ignore'), use_container_width=True, hide_index=True)

        # ⚙️ ABA 9: GERENCIAR ESTOQUES
        with abas_render[7]:
